...
description: "认识 Harvester1、Upgrader1 和 Builder1，理解角色只是玩家为 Creep 安排工作的方式。"
...
> **从这一篇开始统一名称**
> 上一篇的 `Worker1` 是临时示例名。从本篇开始，系列统一使用 `Harvester1`、`Upgrader1` 和 `Builder1`。如果你暂时仍使用 Worker1，可以先把代码中的 Harvester1 换成 Worker1。
...
从 Source 获取 Energy，并把 Energy 送到 Spawn 或 Extension。新手阶段把采集和运输放在同一只 Creep 身上。
...
| 概念 | 最简单的理解 |
| --- | --- |
| 身体部件 | 决定 Creep 能不能移动、采集、携带或工作 |
| 角色 | 决定你希望这只 Creep 主要做哪一类任务 |

例如，Upgrader1 和 Builder1 都可以使用 `[WORK, CARRY, MOVE]`，但代码会让它们前往不同目标。

## 四、为什么先用固定名称区分工作

专业代码通常会使用 Memory 和角色字段管理大量 Creep，但这不是新手当前必须学习的内容。
...
1. 是否有人持续向 Spawn 或 Extension 送 Energy；
2. 是否有人持续升级 Controller；
3. 出现 Construction Site 后，是否有人负责建造；
4. 三只 Creep 是否长期挤在同一个目标旁边。

**一个角色可以有多只 Creep 吗？**

可以，但本系列暂时每种角色只使用一个固定名称，先把基本过程看懂。

**角色是 Screeps 官方固定功能吗？**

不是。角色只是玩家为了组织代码而使用的工作名称。

## 总结

> Harvester1 负责能量运输 → Upgrader1 负责升级 → Builder1 负责建造和维修

下一篇将先创建 Upgrader1，再让它自动采集并升级 Controller。
