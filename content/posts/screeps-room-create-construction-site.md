...
description: "使用一次性Memory请求检查房间视野、0—49坐标、墙体地形、已有结构、已有工地与账号100个工地上限，再保存createConstructionSite()返回值。"
...
verification:
  ...
  testedAt: "2026-07-22"
  testEnvironment: "Node.js 24 离线模拟（请求参数、坐标、地形、占用对象与工地数量，不是 Screeps 官方服务器）"
  testResult: "房间不可见、坐标越界、墙体地形、已有结构、已有工地、账号工地上限和可提交场景通过。"
featured: false
---

`Room.createConstructionSite()` 会在指定位置安排一个 Construction Site。它不会直接生成 Road，也不会替 Builder 完成施工。

本文只解决一个问题：怎样通过一次性请求在明确坐标放置 Road 工地，并避免每 tick 重复调用、坐标错误和目标格已被占用。
...
3. 目标格不是自然墙体；
4. 目标格没有已有 Road；
5. 目标格没有已有 Construction Site；
6. 账号工地数量尚未达到100；
7. 返回值被保存；
8. 下一 tick 再检查工地是否出现。

其中地形和占用检查只是提前给出更清楚的原因。最终是否允许放置仍以API返回值为准。
...
本文只允许 `STRUCTURE_ROAD`，避免把一个简单示例变成可放置任意建筑的蓝图系统。
...
function evaluateRoadSiteRequest(input) {
  ...
  if (terrain === TERRAIN_MASK_WALL) {
    return { ready: false, reason: 'terrain-wall' };
  }
...
`100`来自当前官方 `ERR_FULL`说明。若未来规则改变，最终仍应以API返回值为准。
...
function handleConstructionSiteRequest() {
  ...
  if (room && coordinatesValid) {
    terrain = room.getTerrain().get(request.x, request.y);
...
房间暂时不可见、目标格被占用或账号工地已满时，持续保留开启状态会让旧请求在条件变化后突然执行。

示例把请求状态保存为：

```text
precheck-room-not-visible
precheck-terrain-wall
...
| `ERR_INVALID_TARGET` | 该位置不能放置结构 | 地形、结构、工地与特殊对象 |
...
找到 `STRUCTURE_ROAD` 的工地，才说明后续状态中对象已经出现。Builder仍需要携带Energy并调用 `build()`。
...
离线模拟没有调用真实 `createConstructionSite()`，也没有复制全部RCL与布局规则。
