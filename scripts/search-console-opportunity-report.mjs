...
const report = [
  ...
  "",
  "| Priority | Page or query | Query | Lang | System | Module / roadmap | Stage | Owner keyword | Owner status | Clicks | Impressions | CTR | Position | Recommended action |",
  "|---|---|---|---|---|---|---|---|---|---:|---:|---:|---:|---|",
  ...records.map((record) => `| ${record.priority} | ${record.subject.replaceAll("|", "\\|")} | ${record.query.replaceAll("|", "\\|")} | ${record.language} | ${record.system} | ${record.module} | ${record.stage} | ${record.ownerKeyword} | ${record.ownerStatus} | ${record.clicks} | ${record.impressions} | ${formatGscCtr(record.ctr)} | ${record.position === null ? "—" : record.position.toFixed(1)} | ${record.action} |`),
  "",
