...
function extractToolCatalog(root) {
  ...
  for (const match of source.matchAll(pattern)) {
    records.push({ slug: match[1], title: match[2], description: match[3] });
  }
  return records;
}
...
function extractDiagnosticSymptoms(root) {
  ...
  const pattern = /\n\s*id:\s*"([^"]+)"[\s\S]*?\n\s*zhTitle:\s*"([^"]+)"[\s\S]*?\n\s*enTitle:\s*"([^"]+)"/g;
  for (const match of source.matchAll(pattern)) {
    records.push({ id: match[1], title: match[2], enTitle: match[3] });
  }
  return records;
}
...
function extractApiHubs(root) {
  ...
  const pattern = /\n\s*slug:\s*"([^"]+)"[\s\S]*?\n\s*objectName:\s*"([^"]+)"[\s\S]*?\n\s*zhTitle:\s*"([^"]+)"/g;
  for (const match of source.matchAll(pattern)) {
    records.push({ slug: match[1], objectName: match[2], title: match[3] });
  }
  return records;
}
...
function withDecisionHooks(asset) {
  return {
    ...asset,
    joinKeys: {
      path: asset.path,
      ownerKeyword: asset.primaryKeyword ?? null,
    },
    decision: emptyDecisionState(),
  };
}
...
export function buildSiteAssetMaster(root = process.cwd()) {
  ...
  const articleAssets = contentIndex.assets.map((asset) => {
    const knowledgeRecord = knowledgeBySlug.get(asset.slug);
    const roadmapRecord = roadmapBySlug.get(asset.slug);
    const isKnowledge = asset.system === "knowledge";
    const order = isKnowledge ? knowledgeRecord?.knowledge?.order ?? null : roadmapRecord?.roadmap?.order ?? null;
    const parentPath = isKnowledge ? `/knowledge/${asset.module}` : "/beginner";

    return withDecisionHooks({
      assetId: `zh-CN:article:${asset.slug}`,
      assetType: "article",
      language: "zh-CN",
      path: asset.href,
      canonicalPath: asset.href,
      routeKind: "page",
      slug: asset.slug,
      title: articleTitle(root, asset.slug),
      contentSystem: asset.system,
      module: isKnowledge ? asset.module : null,
      roadmap: isKnowledge ? null : asset.module,
      stage: asset.stage || null,
      order,
      difficulty: asset.difficulty || null,
      primaryKeyword: asset.ownerKeyword || null,
      keywordRole: asset.keywordRole || null,
      searchIntent: asset.searchIntent || null,
      metadataSource: asset.metadataSource || null,
      sourceOfTruth: asset.metadataSource || "generated-registry",
      parentPath,
    });
  });
  ...
  const toolAssets = extractToolCatalog(root).map((tool, index) => withDecisionHooks({
    assetId: `zh-CN:tool:${tool.slug}`,
    assetType: "tool",
    language: "zh-CN",
  ...
  const diagnosticAssets = extractDiagnosticSymptoms(root).map((symptom, index) => withDecisionHooks({
    assetId: `zh-CN:diagnostic:${symptom.id}`,
    ...
    keywordRole: null,
    searchIntent: null,
    metadataSource: "diagnostic-registry",
    sourceOfTruth: "src/lib/screeps-diagnostic-symptoms.ts",
    parentPath: "/diagnostics",
  }));

  const apiHubAssets = extractApiHubs(root).map((hub, index) => withDecisionHooks({
    assetId: `zh-CN:api-hub:${hub.slug}`,
    assetType: "api-hub",
    language: "zh-CN",
    path: `/screeps-api/${hub.slug}`,
    canonicalPath: `/screeps-api/${hub.slug}`,
    routeKind: "page",
    slug: hub.slug,
    title: hub.title,
    contentSystem: "reference",
    module: hub.objectName,
    roadmap: null,
    stage: null,
    order: (index + 1) * 10,
    difficulty: null,
    ...
    keywordRole: null,
    searchIntent: null,
    metadataSource: "api-hub-registry",
    sourceOfTruth: "src/lib/screeps-api-hubs.ts",
  ...
  for (const asset of assets) {
    const records = byPath.get(asset.path) ?? [];
    records.push(asset);
    byPath.set(asset.path, records);
  }
