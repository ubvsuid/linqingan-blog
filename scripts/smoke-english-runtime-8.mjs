...
const articles = [
  {
    ...
    query: "Game.cpu.getUsed", tocId: "use-this-guide", tocHeading: "Use this guide when", faqExpected: false, modifiedExpected: true,
  ...
  },
  {
    ...
    query: "global cache", tocId: "use-this-guide", tocHeading: "Use this guide when", faqExpected: false, modifiedExpected: true,
  ...
  },
  {
    ...
    query: "RawMemory Segments", tocId: "use-this-guide", tocHeading: "Use this guide when", faqExpected: false, modifiedExpected: true,
    signals: ["request-now, read-later lifecycle", "RawMemory.setActiveSegments(active)", "raw === undefined", "100 * 1024", "Live multi-tick verification", "Pending"],
  },
];
const failures = [];
for (const article of articles) {
  ...
  if (article.modifiedExpected && !body.includes(`"dateModified":"2026-07-31"`)) failures.push(`${article.path}: missing 2026-07-31 dateModified`);
...
if (!segmentBody.includes("requests.clear()") || !segmentBody.includes("slice(0, 10)") || !segmentBody.includes("segment-unavailable")) failures.push("Segments page is missing consolidated activation or unavailable-state handling");
