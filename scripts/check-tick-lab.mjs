...
const cases = [
  {
    ...
    expected: { returnName: "OK", returnCode: 0, transferred: 50, nextCreepEnergy: 0, nextTargetFreeCapacity: 0 },
  },
  {
    name: "partial transfer when target has less free capacity",
    input: { range: 1, creepEnergy: 50, targetFreeCapacity: 20 },
    expected: { returnName: "OK", returnCode: 0, transferred: 20, nextCreepEnergy: 30, nextTargetFreeCapacity: 0 },
  },
  {
    name: "range failure",
    input: { range: 2, creepEnergy: 50, targetFreeCapacity: 50 },
    expected: { returnName: "ERR_NOT_IN_RANGE", returnCode: -9, transferred: 0, nextCreepEnergy: 50, nextTargetFreeCapacity: 50 },
  },
  {
    name: "resource failure",
    input: { range: 1, creepEnergy: 0, targetFreeCapacity: 50 },
    expected: { returnName: "ERR_NOT_ENOUGH_RESOURCES", returnCode: -6, transferred: 0, nextCreepEnergy: 0, nextTargetFreeCapacity: 50 },
  },
  {
    name: "capacity failure",
    input: { range: 1, creepEnergy: 50, targetFreeCapacity: 0 },
    expected: { returnName: "ERR_FULL", returnCode: -8, transferred: 0, nextCreepEnergy: 50, nextTargetFreeCapacity: 0 },
  },
  {
    name: "range is checked before resources in the constrained model",
    input: { range: 3, creepEnergy: 0, targetFreeCapacity: 0 },
    expected: { returnName: "ERR_NOT_IN_RANGE", returnCode: -9, transferred: 0, nextCreepEnergy: 0, nextTargetFreeCapacity: 0 },
  },
];
...
console.log(`Tick Lab check passed: ${cases.length} deterministic transfer cases + route/discovery contracts.`);
