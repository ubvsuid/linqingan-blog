...
import { getPublicTagRecords } from "@/lib/tags";
...
export const metadata = createPageMetadata({
  title: "Screeps 知识库",
  description:
    "从 Screeps 新手路线进入 8 个专题模块，并查询 API、术语、错误码、标签、验证方法、房间诊断、Creep 身体计算器和全部站内内容。",
  path: "/knowledge",
});
...
const referenceTools = [
  {
    ...
    description:
      "按对象、方法或关键词快速定位常用 Game、Creep、Room、Structure 与系统 API。",
    href: "/screeps-api",
    count: `${screepsApiReference.length} 个常用入口`,
  },
  {
    eyebrow: "ROOM DIAGNOSTICS",
    title: "房间运行诊断",
    description:
      "按 Spawn、角色、Energy、Controller、工地和 CPU 快照定位常见风险。",
  ...
  },
  {
    ...
    title: "Creep 身体计算器",
    description:
      "组合身体部件，计算 Energy 成本、生成时间、携带容量和满载移动速度。",
  ...
  },
  {
    ...
    title: "站内搜索",
    description:
      "同时搜索文章摘要、知识模块、术语、错误码、工具和公开建设说明。",
    href: "/search",
  ...
  },
  {
    ...
    title: "Screeps 术语表",
    description:
      "用新手能理解的方式解释 Creep、Spawn、Memory、RCL 等常见概念。",
    href: "/glossary",
  ...
  },
  {
    ...
    title: "错误码查询",
    description:
      "查询 ERR_NOT_IN_RANGE、ERR_FULL、ERR_INVALID_TARGET 等返回值的原因与处理方式。",
    href: "/screeps-errors",
  ...
  },
  {
    ...
    title: "文章标签",
    description:
      "按照 Creep、JavaScript、Spawn、Memory 等长期主题浏览已发布内容。",
    href: "/tags",
    count: `${getPublicTagRecords().length} 个可浏览标签`,
  },
  {
    eyebrow: "VERIFICATION",
    title: "文章验证方法",
    description:
      "了解文档核对、语法检查、离线模拟、Console 与真实主循环验证的区别。",
    href: "/verification",
    count: "5 种验证状态",
  },
];
    ...
    "@graph": [
      {
        ...
        name: "Screeps 知识库",
        description:
          "Screeps 中文新手路线、专题模块、API、术语、错误码、验证方法和实用工具入口。",
        url: pageUrl,
        inLanguage: "zh-CN",
      ...
      <Container>
        <header className={`page-header ${styles.header}`}>
          ...
          <div className={styles.stats} aria-label="知识库数据">
            <span>
              <strong>{beginnerSeriesSlugs.length}</strong> 篇新手路线
            </span>
            <span>
              <strong>{knowledgeBaseSlugs.length}</strong> 篇专题文章
            </span>
            <span>
              <strong>{knowledgeBaseSections.length}</strong> 个独立模块
            </span>
          </div>
        ...
        <section
          ...
          aria-labelledby="knowledge-beginner-title"
        >
          <div>
            ...
            <Link href="/beginner">
              开始 {beginnerSeriesSlugs.length} 篇新手路线 →
            </Link>
          </div>
        ...
        <section
          ...
          aria-labelledby="knowledge-reference-title"
        >
          <div className={styles.sectionHeading}>
            ...
            <p>
              文章用于连续学习；这些入口用于遇到具体 API、名词、返回值、身体配置或代码问题时快速定位。
            </p>
          </div>
            ...
            return (
              ...
              >
                <header className={styles.moduleHeader}>
                  ...
                  <div>
                    <div className={styles.moduleTitleRow}>
                      <h2>
                        <Link href={`/knowledge/${section.id}`}>
                          {section.title}
                        </Link>
                      </h2>
                      <small>
                        {section.slugs.length} 篇 · {section.stages.length} 个阶段
                      </small>
                    </div>
                  ...
                  {previewPosts.map((post, index) => (
                    <li key={post.slug}>
                      ...
                      <Link href={`/blog/${post.slug}`}>
                        ...
                        <small>
                          {post.category} · {post.readingMinutes} 分钟
                        </small>
                      </Link>
                    </li>
                  ))}
                ...
                <div className={styles.moduleFooter}>
                  <Link href={`/knowledge/${section.id}`}>
                    查看本模块全部 {section.slugs.length} 篇 →
                  </Link>
                </div>
              </section>
            );
