...
export default async function BlogPostLayout({
  ...
  const previousPost = position.previousSlug
    ? postsBySlug.get(position.previousSlug)
    : null;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      },
      {
        "@type": "ListItem",
        position: 3,
        name: position.section.title,
        item: `${siteConfig.url}${moduleHref}`,
      },
      {
        "@type": "ListItem",
        position: 4,
        name: currentPost?.title ?? slug,
        item: articleUrl,
      },
    ],
  };
  ...
  return (
    <>
      ...
      <div className="knowledge-article-context">
        <Container>
          <nav className="knowledge-article-breadcrumb" aria-label="面包屑">
            ...
            <span aria-hidden="true">/</span>
            <Link href={moduleHref}>{position.section.title}</Link>
            <span aria-hidden="true">/</span>
          ...
          <div className="knowledge-article-module-row">
            <div>
              ...
              <Link className="knowledge-article-module-title" href={moduleHref}>
            ...
            <Link className="knowledge-article-position" href={moduleHref}>
              <span>专题内进度</span>
              <strong>{position.index + 1} / {position.section.slugs.length}</strong>
              <small>查看完整学习顺序 →</small>
            </Link>
          </div>
        </Container>
      </div>
      ...
      <div className="knowledge-article-navigation">
        <Container>
          <div className="knowledge-article-navigation-heading">
            <div>
              <p className="eyebrow">CONTINUE THIS MODULE</p>
              <h2>继续学习{position.section.title}</h2>
            </div>
      ...
      <style>{`
        ...
        .article-shell > .article-container > article > .article-pagination {
          display: none;
        }
