...
const publishedLabel = "August 17, 2026";
...
<h2 id="utf8-budget">Budget UTF-8 bytes, not JavaScript length</h2><p>The official RawMemory documentation limits each segment to 100 KB, but the reviewed API text does not define the server-side byte-accounting algorithm. This guide uses UTF-8 counting only for its conservative 96 KiB project budget; 96 KiB is not presented as another official Screeps limit.</p><pre><code class="language-javascript">function utf8ByteLength(value) {
...
<h2 id="normalize-public">Normalize the public ID list</h2><p>Later <code>setPublicSegments()</code> calls replace earlier lists. The official documentation requires IDs from 0 through 99 but does not list duplicate IDs as a separate API error, so this coordinator validates every ID, deduplicates, sorts, and submits one complete desired list.</p><pre><code class="language-javascript">function normalizePublicSegmentIds(ids) {
...
<h2 id="public-policy">Submit one coherent public policy</h2><p>This guide adds a <strong>project policy</strong>: when <code>defaultId</code> is non-null, require it to appear in the same desired public list. The official API documents <code>setPublicSegments()</code> and <code>setDefaultPublicSegment()</code> separately; this membership check is a local configuration invariant, not an extra Screeps return-code rule. Use an empty array to expose no segments and <code>null</code> to remove the default.</p><pre><code class="language-javascript">function applyPublicSegmentPolicy({
  publicIds,
  defaultId
}) {
  const normalized =
