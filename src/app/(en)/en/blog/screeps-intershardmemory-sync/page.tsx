...
<pre><code class="language-javascript">let interShardDraftTick = null;
...
<p><code>setLocal()</code> has no documented <code>OK</code> return code. The coordinator above also keeps the newest envelope in a tick-scoped draft, so later channel updates merge that staged document instead of assuming a documented <code>setLocal()</code>-then-<code>getLocal()</code> read-after-write contract. The status <code>local-write-called</code> therefore describes only the local function call. It is not evidence that another shard has already observed the new revision.</p>
...
<p><code>buildOutboundHandoff()</code> only creates one offer record. The source coordinator should merge it into the existing <code>handoffOffers</code> map, apply <code>pruneRecordMap()</code>, and publish the bounded map from its own shard rather than replacing other unresolved offers.</p>
<pre><code class="language-javascript">function acknowledgeRemoteHandoff(
  offer,
  observedCreep
) {
  if (
