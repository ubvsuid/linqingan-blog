...
export const metadata = createPageMetadata({
  title: "Screeps 资料中心",
  description: "查询 Screeps 常见术语、API 返回值和文章标签，快速定位新手学习中遇到的问题。",
  path: "/resources",
});

const resources = [
  { eyebrow: "GLOSSARY", title: "Screeps 术语表", description: "用新手能理解的方式解释 Creep、Spawn、Memory、RCL 等常见概念。", href: "/glossary", count: `${screepsGlossary.length} 个术语` },
  { eyebrow: "ERROR CODES", title: "错误码查询", description: "查询 ERR_NOT_IN_RANGE、ERR_FULL、ERR_INVALID_TARGET 等返回值的原因和处理方式。", href: "/screeps-errors", count: `${screepsErrorCodes.length} 个返回值` },
  { eyebrow: "TAGS", title: "文章标签", description: "按照 Creep、JavaScript、Spawn、Builder 等主题浏览已经发布的内容。", href: "/tags", count: `${getTagRecords().length} 个标签` },
];
    ...
    <main className="page-shell resources-page">
      <Container>
        <header className="page-header resources-header">
          ...
          <h1>资料中心</h1>
          <p>文章负责连续学习，资料中心负责在遇到问题时快速查询。这里会逐步整理 Screeps 术语、错误码、常用 API 和实用工具。</p>
        </header>
        ...
        <div className="resources-grid">
          {resources.map((resource) => (
            <Link key={resource.href} href={resource.href}>
              ...
              <div><span>{resource.count}</span><strong aria-hidden="true">→</strong></div>
            </Link>
          ))}
        </div>

        <section className="resource-roadmap" aria-labelledby="resource-roadmap-title">
          <div><p className="eyebrow">NEXT</p><h2 id="resource-roadmap-title">接下来会加入</h2></div>
          <ol>
            <li><span>01</span><div><strong>常用 API 快速查询</strong><p>只整理新手和基础工程阶段真正会使用的对象、方法、参数与返回值。</p></div></li>
            <li><span>02</span><div><strong>Creep 身体计算器</strong><p>输入身体部件后计算能量成本、创建时间和基础移动比例。</p></div></li>
            <li><span>03</span><div><strong>基础工程学习路线</strong><p>从固定名称代码继续走向 Memory、自动补员、模块拆分和房间管理。</p></div></li>
          </ol>
        </section>
      </Container>
      <style>{`
        ...
        .resources-grid > a { display: grid; min-height: 310px; align-content: start; border: 1px solid var(--border); border-radius: 22px; padding: 28px; background: var(--surface); transition: transform 160ms ease, border-color 160ms ease; }
