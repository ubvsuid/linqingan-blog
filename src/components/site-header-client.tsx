...
interface SiteHeaderClientProps {
  chineseNavigation: NavigationItem[];
  englishNavigation: NavigationItem[];
  beginnerArticlePaths: string[];
  knowledgeArticlePaths: string[];
  languageRoutePairs: Array<[string, string]>;
}
...
function getLanguageSwitchTarget(
  ...
  if (isEnglishPath(pathname)) {
    const match = routePairs.find(([, englishPath]) => pathname === englishPath);
    return match?.[0] ?? "/";
  }

  const normalized = pathname === "/" ? "/" : pathname.replace(/\/$/, "");
  const match = routePairs.find(
    ([chinesePath]) => normalized === chinesePath.split("#")[0],
  );
  return match?.[1] ?? "/en";
}
...
export function SiteHeaderClient({
  chineseNavigation,
  englishNavigation,
  beginnerArticlePaths,
  knowledgeArticlePaths,
  languageRoutePairs,
  ...
  const navigation = english ? englishNavigation : chineseNavigation;
  const languageTarget = getLanguageSwitchTarget(pathname, languageRoutePairs);
  const isBeginnerArticle = beginnerArticlePaths.includes(pathname);
  const isKnowledgeArticle = knowledgeArticlePaths.includes(pathname);
        ...
        >
          ...
          {english ? (
            <span className="brand-copy">
              <strong>Linqingan</strong>
              <small>Screeps Guides &amp; Tools</small>
            </span>
          ) : null}
        </Link>
