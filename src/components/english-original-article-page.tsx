...
interface EnglishOriginalArticlePageProps {
  articleHref: string;
  headline: string;
  description: string;
...
export function EnglishOriginalArticlePage({
  articleHref,
  headline,
  description,
  breadcrumbLabel,
  ...
  previous,
  next,
}: EnglishOriginalArticlePageProps) {
  ...
  const articleSlug = articleHref.split("/").filter(Boolean).at(-1) ?? "english-guide";
    ...
    <main className="article-shell" lang="en">
      <Container className="article-container">
        ...
        <article>
          ...
          <EnglishArticleLearningTracker href={articleHref} title={headline} />
          ...
          <section className="english-source-panel" aria-labelledby={`english-source-title-${articleSlug}`}>
            <div>
              ...
              <h2 id={`english-source-title-${articleSlug}`}>Review the evidence or continue the system</h2>
              <p>This is an original English guide. Its verification record separates documentation-reviewed claims from Console or live-shard observations that have not been performed.</p>
            </div>
            <div>
          ...
          <ArticleFeedback
            slug={articleSlug}
            title={headline}
            articleUrl={`${siteConfig.url}${articleHref}`}
