    ...
    <main className="article-shell" lang="en">
      <Container className="article-container">
        ...
        <article>
          <header className="article-header">
            ...
            {discovery ? (
              ...
              >
                ...
                <Link href={`/en/blog?type=${discovery.contentType}`}>
            ...
            <div className="tag-list" aria-label="Article topics">
              {discovery
                ? discovery.tags.map((tag, index) => (
                    <Link
                      className="tag"
                      href={`/en/tags/${discovery.tagSlugs[index]}`}
                      key={tag}
                    >
                      {tag}
                    </Link>
          ...
          <details className="english-verification">
            <summary>
              <span>
                ...
                <small>
                  {verificationSummary || "Open the full verification record"}
                </small>
              </span>
          ...
          <section
            className="english-source-panel"
            aria-labelledby="english-source-title"
          >
            <div>
              <p className="eyebrow">SOURCE AND SCOPE</p>
              <h2 id="english-source-title">
                Review the Chinese source or continue by topic
              </h2>
              <p>
                This English guide is rewritten for a focused search intent while
                preserving the technical scope and verification boundaries of its
                Chinese source.
              </p>
            </div>
            <div>
              <Link href={resolvedChinesePath} hrefLang="zh-CN">
          ...
          {related.length > 0 ? (
            <section
              className="english-related"
              aria-labelledby="english-related-title"
            >
              <div>
                <p className="eyebrow">RELATED GUIDES</p>
                <h2 id="english-related-title">Continue with a related problem</h2>
              </div>
              <div>
                {related.map((article) => (
                  <Link href={article.href} key={article.href}>
                    ...
                    <small>
                      {article.readingTime} · {article.difficulty}
                    </small>
                  </Link>
                ))}
              </div>
