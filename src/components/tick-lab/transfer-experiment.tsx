...
interface TransferExperimentProps {
...
const copy = {
  ...
  en: {
    ...
    related: "Continue with Transfer",
    guide: "Full guide",
    api: "Creep API",
    errors: "Return codes",
    diagnostics: "Diagnostics",
...
export function TransferExperiment({ language }: TransferExperimentProps) {
  const text = copy[language];
  ...
  const presets: ScenarioPreset[] = [
    { key: "success", scenario: { range: 1, creepEnergy: 50, targetFreeCapacity: 50 }, label: text.presetSuccess, detail: text.presetSuccessDetail },
    { key: "partial", scenario: { range: 1, creepEnergy: 50, targetFreeCapacity: 25 }, label: text.presetPartial, detail: text.presetPartialDetail },
    { key: "range", scenario: { range: 2, creepEnergy: 50, targetFreeCapacity: 50 }, label: text.presetRange, detail: text.presetRangeDetail },
    { key: "empty", scenario: { range: 1, creepEnergy: 0, targetFreeCapacity: 50 }, label: text.presetEmpty, detail: text.presetEmptyDetail },
    { key: "full", scenario: { range: 1, creepEnergy: 50, targetFreeCapacity: 0 }, label: text.presetFull, detail: text.presetFullDetail },
  ];
  ...
  function runTick() {
    setResult(evaluateTransfer({ range, creepEnergy, targetFreeCapacity }));
  }
  ...
  return (
    <section className={styles.labSection} aria-labelledby="transfer-lab-title">
      ...
      <div className={styles.stageGrid}>
        <article className={styles.panel}>
          ...
          <div className={styles.objectCard}>
            <div className={styles.objectHeader}><strong>{text.creep}</strong><code>Creep</code></div>
            <dl>
      ...
      <section className={styles.presetPanel} aria-labelledby="transfer-presets-title">
        <div className={styles.presetHeader}>
          <div><strong id="transfer-presets-title">{text.presets}</strong><p>{text.presetsLead}</p></div>
        </div>
      ...
      <div className={styles.controlGrid}>
        <fieldset className={styles.controlCard}>
          ...
          <div className={styles.rangeButtons}>
            {[1, 2, 3].map((value) => (
              <button key={value} type="button" className={range === value ? styles.rangeActive : undefined} aria-pressed={range === value} onClick={() => { setRange(value); invalidateResult(); }}>
                {value}
              </button>
            ))}
          </div>
        ...
        <label className={styles.controlCard}>
          <span>{text.creepEnergy}</span><output>{creepEnergy} / 50</output>
          <input type="range" min="0" max="50" step="10" value={creepEnergy} onChange={(event) => { setCreepEnergy(Number(event.target.value)); invalidateResult(); }} />
        </label>
        <label className={styles.controlCard}>
          <span>{text.targetFree}</span><output>{targetFreeCapacity} / 300</output>
          <input type="range" min="0" max="300" step="25" value={targetFreeCapacity} onChange={(event) => { setTargetFreeCapacity(Number(event.target.value)); invalidateResult(); }} />
        </label>
      </div>
      ...
      <div className={styles.timelineResultGrid}>
        <section className={styles.timelinePanel} aria-labelledby="transfer-timeline-title">
          <div className={styles.panelLabel} id="transfer-timeline-title">{text.timeline}</div>
          {result ? (
        ...
        <section className={styles.resultPanel} aria-live="polite" aria-labelledby="transfer-result-title">
          <div className={styles.panelLabel} id="transfer-result-title">{text.result}</div>
          {result ? (
            <>
              ...
              <dl className={styles.resultStats}>
                ...
                <div><dt>{text.transferred}</dt><dd>{result.transferred} ENERGY</dd></div>
              </dl>
