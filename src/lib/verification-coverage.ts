...
export const verificationCoveragePlans: readonly VerificationCoveragePlan[] = [
  ...
  },
  {
    ...
    primaryApiEntryIds: ["creep-withdraw", "creep-pickup", "creep-transfer", "creep-move-to"],
    ...
    zhNextEvidence: "分别保存 withdraw、pickup、moveTo 与 transfer 返回值，并连续记录 source、Creep、destination Store，定位物流链真正断在哪一步。",
    enNextEvidence: "Store withdraw, pickup, moveTo, and transfer results separately and track source, Creep, and destination Stores across ticks to locate the broken logistics stage.",
  },
  {
    symptomId: "builder-not-building",
    priority: "P0",
    targetLevel: "live-multitick",
    primaryErrorNames: ["ERR_NOT_IN_RANGE", "ERR_NOT_ENOUGH_RESOURCES", "ERR_INVALID_TARGET", "ERR_NO_BODYPART"],
    primaryApiEntryIds: ["creep-build", "creep-repair"],
    zhGoal: "覆盖 build()/repair() 的返回值、WORK、Energy、目标类型与后续 Construction Site progress / structure hits 变化。",
    enGoal: "Cover build()/repair() results, WORK parts, Energy, target type, and later Construction Site progress or structure hits changes.",
    zhNextEvidence: "记录 build/repair 返回值、Creep Energy、WORK 部件、目标初始状态和至少一个后续 tick 的 progress 或 hits。",
    enNextEvidence: "Record build/repair results, Creep Energy, WORK parts, target starting state, and at least one later tick of progress or hits.",
  },
  {
    symptomId: "tower-not-acting",
    priority: "P0",
    targetLevel: "live-multitick",
    primaryErrorNames: ["ERR_NOT_ENOUGH_RESOURCES", "ERR_INVALID_TARGET"],
    primaryApiEntryIds: ["tower-attack-heal-repair", "room-get-event-log"],
    zhGoal: "覆盖 Tower attack/heal/repair 的返回值、Energy、目标与后续状态；距离只作为效果计算因素，不替代真实运行证据。",
    enGoal: "Cover Tower attack/heal/repair results, Energy, targets, and later state; treat range as an output factor rather than a substitute for runtime evidence.",
    zhNextEvidence: "优先使用可控维修或治疗场景记录动作返回值、Tower Energy、目标 hits 与后续 tick；攻击场景只在自然出现且可明确记录时补充。",
    enNextEvidence: "Prefer controlled repair or heal cases with action result, Tower Energy, target hits, and a later tick; add attack evidence only when it occurs naturally and can be recorded clearly.",
  },
] as const;
