...
export function applyEnglishEditorialNinth20260817(
  ...
  if (!article || !SELECTED_SLUGS.has(article.slug)) {
    return article;
  }

  if (article.slug === "screeps-cpu-getused-bucket") {
    return improveCpu(article);
  }
  if (article.slug === "screeps-memory-basics") {
    return improveMemory(article);
  }
  if (article.slug === "screeps-spawncreep-return-codes") {
    return improveSpawnReturnCodes(article);
  }
  if (article.slug === "screeps-moveto-not-moving") {
    return improveMovement(article);
  }
