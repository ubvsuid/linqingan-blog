...
export const knowledgeBaseSections: readonly KnowledgeBaseSection[] = [
  ...
  },
  {
    ...
    description: "从创建、身体配置和返回值排查，到提前补员、续命、回收与房间断代恢复，管理 Creep 的完整生命周期。",
    audience: "已经会创建基础 Creep，希望减少重名、Energy 不足和房间断代问题的玩家。",
    learningGoal: "把单次 spawnCreep() 调用扩展为可诊断、可提前交接、可恢复的 Creep 创建与生命周期管理流程。",
    stages: [
      { title: "创建与身体", description: "先读懂创建返回码，再根据可用 Energy 生成身体。", from: 0, to: 2 },
      { title: "提前补员与续命", description: "先在旧 Creep 死亡前生成替代者，再理解 renewCreep() 和 recycleCreep() 的适用边界。", from: 2, to: 5 },
      { title: "断代恢复", description: "最后处理房间没有工作 Creep 时的紧急恢复。", from: 5, to: 6 },
    ],
    slugs: [
  ...
  },
  {
    ...
    audience: "基础采集循环已经运行，希望减少空跑并建立房间资源链路的玩家。",
    learningGoal: "理解资源从 Source 或掉落物进入 Creep，再流向 Container、Storage、Link 和 Terminal 的全过程。",
    stages: [
      { title: "获取资源", description: "从 Container、掉落 Energy 和多 Source 选择开始。", from: 0, to: 3 },
      { title: "房间内储运", description: "理解 Storage 和 Link 在房间经济中的位置。", from: 3, to: 5 },
      { title: "跨房间与矿物", description: "继续学习 Terminal 发送和 Mineral 采集。", from: 5, to: 7 },
    ],
    slugs: [
