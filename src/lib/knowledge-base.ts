...
export const knowledgeBaseSections: readonly KnowledgeBaseSection[] = [
  ...
  },
  {
    ...
    description: "从创建、身体配置、房间 Energy 供能与出生出口诊断，到提前替换、续命、回收和断代恢复，管理 Creep 的完整生命周期。",
    audience: "已经会创建基础 Creep，希望减少重名、Energy 不足、Extension 未填满、出生阻塞、换代空窗和房间断代问题的玩家。",
    learningGoal: "把单次 spawnCreep() 调用扩展为可诊断、可放行、可提前交接和可恢复的 Creep 创建、供能与生命周期管理流程。",
    stages: [
      { title: "创建、身体、供能与出口", description: "先读懂创建返回码，根据可用 Energy 生成身体，诊断 Spawn 与 Extension 供能，并处理孵化完成后的出口阻塞。", from: 0, to: 4 },
      { title: "提前替换、续命与回收", description: "在旧 Creep 死亡前安排替代者，再理解 renewCreep() 和 recycleCreep() 的适用边界。", from: 4, to: 7 },
      { title: "断代恢复", description: "最后处理房间没有工作 Creep 时的紧急恢复。", from: 7, to: 8 },
    ],
    slugs: [
  ...
  },
  {
    ...
    number: 3,
    title: "资源采集与房间经济",
    description: "覆盖 Container 取能与腐化维修、掉落资源、Tombstone、Ruin、Storage、Link、Terminal 与 Mineral，让资源获取、保全、储存和运输形成清楚链路。",
    audience: "基础采集循环已经运行，希望减少空跑、防止 Container 腐化损毁、抢救遗留资源并建立房间资源链路的玩家。",
    learningGoal: "理解资源从 Source、掉落物、Tombstone 或 Ruin 进入 Creep，再流向 Container、Storage、Link 和 Terminal，并保持关键 Container 可持续运行。",
    stages: [
      { title: "Container 获取与维护", description: "先学会从 Container 取能，再根据 ticksToDecay、hits 与到达时间安排腐化维修。", from: 0, to: 2 },
      { title: "获取与回收资源", description: "继续处理掉落 Energy、Tombstone、Ruin 和多 Source 选择。", from: 2, to: 5 },
      { title: "房间内储运", description: "理解 Storage 和 Link 在房间经济中的位置。", from: 5, to: 7 },
      { title: "跨房间与矿物", description: "最后学习 Terminal 发送和 Mineral 采集。", from: 7, to: 9 },
    ],
    slugs: [
