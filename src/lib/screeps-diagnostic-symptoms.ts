...
export const screepsDiagnosticSymptoms = [
  ...
  },
  {
    ...
    zhSummary: "boostCreep() 没有效果时，先保存真实返回值，再按所有权、可强化部件、Lab 资源、目标、距离和 RCL 分支排查；OK 之后再跨 tick 验证真实 body boost 与 Lab Store 变化。",
    enSummary: "When boostCreep() appears to fail, capture the real return value first, then separate ownership, eligible parts, Lab resources, target validity, range, and RCL; after OK, verify body boosts and Lab Store changes on a later tick.",
    zhTriage: [
      "保存 lab.boostCreep(creep, bodyPartsCount) 的真实返回值，不要只看 Creep 外观。",
      "检查 Lab 所有权、mineralType、Mineral/Energy Store，以及目标 Creep 是否仍为同一个对象。",
      "确认目标与 Lab 相邻，并且当前 mineral 能强化目标身上尚未强化的对应 body parts。",
      "返回 OK 后保存 Creep ID 与预期 body-part indexes，在后续 tick 对比 part.boost 和 Lab Mineral/Energy Store delta。",
    ],
    enTriage: [
      "Store the real result from lab.boostCreep(creep, bodyPartsCount) instead of judging by appearance alone.",
      "Check Lab ownership, mineralType, Mineral/Energy Store, and that the target is still the same Creep object.",
      "Confirm the Creep is adjacent and that the Lab mineral can boost eligible unboosted body parts on that Creep.",
      "After OK, preserve the Creep ID and expected body-part indexes, then compare part.boost and Lab Mineral/Energy Store deltas on a later tick.",
    ],
    errorNames: ["ERR_NOT_OWNER", "ERR_NOT_FOUND", "ERR_NOT_ENOUGH_RESOURCES", "ERR_INVALID_TARGET", "ERR_NOT_IN_RANGE", "ERR_RCL_NOT_ENOUGH"],
